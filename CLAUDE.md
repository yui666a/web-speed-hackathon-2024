...
```
bundle-analyze → perf-propose → [実装] → perf-validate → perf-measure
     ↑                                                        |
     └────────────────────────────────────────────────────────┘
```
...
| スキル | トリガー | 内容 |
|--------|---------|------|
| **perf-measure** | 「計測して」「スコアは？」 | Lighthouse CLI + chrome-devtools MCP で4ページ計測 → report.md 出力 |
| **perf-propose** | 「改善案を考えて」「次に何を直す？」 | 計測結果を元にコードベースを調査し、優先度付き施策を proposals.md に出力 |
| **perf-validate** | 「チェックして」「テスト通る？」 | ビルド → バンドルサイズ → レギュレーション → E2E/VRT を一括検証 |
